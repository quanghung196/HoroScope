...
public class Activity_ZodiacCompatibility extends AppCompatActivity {
        ...
        recyclerZodiac.addOnItemTouchListener(new RecyclerItemClickListener(this, recyclerZodiac, new RecyclerItemClickListener.ClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (imgMale.getDrawable() != null) {
                    ...
                    female = zodiac.getZodiacName().toLowerCase() + "-female";
                } else {
                    tvMale.setText(zodiac.getZodiacName());
                    imgMale.setImageResource(zodiac.getZodiacImage());
                    male = zodiac.getZodiacName().toLowerCase() + "-male";
                }
            }
    ...
    private void setAdapter() {
        adapter = new ZodiacPickerFeature_Adapter(zodiacs, this, 1);
    ...
    public void btnCheck(View view) {
        if (imgMale.getDrawable() != null && imgFemale.getDrawable() != null) {
            ...
            try {
                horoScopes = readCompanyJSONFile(this, "zodiac_compatibility.json");
                setAdapter2();
    ...
    public List<HoroScope> readCompanyJSONFile(Context context, String fileName) throws IOException, JSONException {
        ...
        JSONObject jsonName = jsonRoot.getJSONObject(male + "-" + female);
        JSONArray jsonArray = jsonName.getJSONArray("infos");

        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject object = jsonArray.getJSONObject(i);
            String title = object.getString("name");
            String horoscope = object.getString("text");
            int percent = object.getInt("percent");
            HoroScope horoScopee = new HoroScope(title, horoscope, percent);
